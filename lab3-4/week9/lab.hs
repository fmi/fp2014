...
-- Тук правим магия, която е често срещана в Haskell
-- Ама е голяма магия и ще си говорим по-нататък за нея
maybeLast :: [a] -> Maybe a
maybeLast [] = Nothing
maybeLast [x] = Just x
...
-- Намиране на обратния списък чрез fold
-- Това ще го обясним следващия път
reverse' :: [a] -> [a]
reverse' items = foldl (\acc x -> x : acc) [] items

-- Намиране на обратния списък чрез опащкова рекурсия
reverse'' :: [a] -> [a]
